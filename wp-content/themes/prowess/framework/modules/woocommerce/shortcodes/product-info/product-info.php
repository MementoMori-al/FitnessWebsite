<?php
namespace ProwessCore\CPT\Shortcodes\ProductInfo;
...
class ProductInfo implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if(function_exists('vc_map')) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Product Info', 'prowess' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'heading'     => esc_html__( 'Selected Product', 'prowess' ),
							'settings'    => array(
								'sortable'      => true,
								'unique_values' => true
							),
							'admin_label' => true,
							'save_always' => true,
							'description' => esc_html__( 'If you left this field empty then product ID will be of the current page', 'prowess' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'product_info_type',
							'heading'     => esc_html__( 'Product Info Type', 'prowess' ),
							'value'       => array(
								esc_html__( 'Title', 'prowess' )          => 'title',
								esc_html__( 'Featured Image', 'prowess' ) => 'featured_image',
								esc_html__( 'Category', 'prowess' )       => 'category',
								esc_html__( 'Excerpt', 'prowess' )        => 'excerpt',
								esc_html__( 'Price', 'prowess' )          => 'price',
								esc_html__( 'Rating', 'prowess' )         => 'rating',
								esc_html__( 'Add To Cart', 'prowess' )    => 'add_to_cart',
								esc_html__( 'Tag', 'prowess' )            => 'tag',
								esc_html__( 'Author', 'prowess' )         => 'author',
								esc_html__( 'Date', 'prowess' )           => 'date',
							),
							'admin_label' => true
						),
						array(
							...
							'heading'    => esc_html__( 'Product Info Color', 'prowess' ),
						...
						array(
							...
							'value'       => array_flip( prowess_select_get_title_tag( true, array( 'p' => 'p' ) ) ),
							'description' => esc_html__( 'Set title tag for product title element', 'prowess' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Featured Image Proportions', 'prowess' ),
							'value'      => array(
								esc_html__( 'Default', 'prowess' )        => '',
								esc_html__( 'Original', 'prowess' )       => 'full',
								esc_html__( 'Square', 'prowess' )         => 'prowess_select_square',
								esc_html__( 'Landscape', 'prowess' )      => 'prowess_select_landscape',
								esc_html__( 'Portrait', 'prowess' )       => 'prowess_select_portrait',
						...
						array(
							...
							'heading'     => esc_html__( 'Category Tag', 'prowess' ),
							'value'       => array_flip( prowess_select_get_title_tag( true ) ),
							'description' => esc_html__( 'Set category tag for product category element', 'prowess' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Skin', 'prowess' ),
							'value'      => array(
								esc_html__( 'Default', 'prowess' ) => '',
								esc_html__( 'White', 'prowess' )   => 'white',
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Size', 'prowess' ),
							'value'      => array(
								esc_html__( 'Default', 'prowess' ) => '',
							...
							'dependency' => array( 'element' => 'product_info_type', 'value'   => array( 'prowess' ) ),
							'group'      => esc_html__( 'Product Info Style', 'prowess' )
						)
					)
				)
	...
	public function getItemTitleHtml( $params ) {
		...
		if ( ! empty( $title ) ) {
			$html = '<' . esc_attr( $title_tag ) . ' itemprop="name" class="qodef-pi-title entry-title" ' . prowess_select_get_inline_style( $product_info_style ) . '>';
			...
			$html .= '<div class="qodef-pi-category">';
				foreach ( $categories as $cat ) {
					if ( ! empty( $params['category_tag'] ) ) {
						$html .= '<' . esc_attr( $params['category_tag'] ) . ' ' . prowess_select_get_inline_style( $product_info_style ) . '>';
						...
						$html .= '<a itemprop="url" class="qodef-pi-category-item" href="' . esc_url( get_term_link( $cat->term_id ) ) . '" ' . prowess_select_get_inline_style( $product_info_style ) . '>' . esc_html( $cat->name ) . '</a>';
					}
				}
			$html .= '</div>';
		}
	...
	public function getItemExcerptHtml( $params ) {
		...
		if ( ! empty( $excerpt ) ) {
			$html = '<div class="qodef-pi-excerpt"><p itemprop="description" class="qodef-pi-excerpt-item" ' . prowess_select_get_inline_style( $product_info_style ) . '>' . esc_html( $excerpt ) . '</p></div>';
		}
	...
	public function getItemPriceHtml( $params ) {
		...
		if ( $price_html = $product->get_price_html() ) {
			$html = '<div class="qodef-pi-price" ' . prowess_select_get_inline_style( $product_info_style ) . '>' . $price_html . '</div>';
		}
	...
	public function getItemRatingHtml( $params ) {
		...
		if ( get_option( 'woocommerce_enable_review_rating' ) !== 'no' ) {
			...
			$html = '<div class="qodef-pi-rating" title="' . sprintf( esc_attr( "Rated %s out of 5", "prowess" ), $average ) . '" ' . prowess_select_get_inline_style( $product_info_style ) . '><span style="width:' . ( ( $average / 5 ) * 100 ) . '%"></span></div>';
		}
	...
	public function getItemAddToCartHtml( $params ) {
		...
		$html .= apply_filters( 'prowess_select_product_info_add_to_cart_link',
	...
	public function getItemTagHtml( $params ) {
		...
		if ( ! empty( $tags ) ) {
			$html = '<div class="qodef-pi-tag">';
				foreach ( $tags as $tag ) {
					$html .= '<a itemprop="url" class="qodef-pi-tag-item" href="' . esc_url( get_term_link( $tag->term_id ) ) . '" ' . prowess_select_get_inline_style( $product_info_style ) . '>' . esc_html( $tag->name ) . '</a>';
				}
			$html .= '</div>';
		}
	...
	public function getItemAuthorHtml( $params ) {
		...
		if ( ! empty( $author ) ) {
			$html = '<div class="qodef-pi-author" ' . prowess_select_get_inline_style( $product_info_style ) . '>' . esc_html( $author ) . '</div>';
		}
	...
	public function getItemDateHtml( $params ) {
		...
		if ( ! empty( $date ) ) {
			$html = '<div class="qodef-pi-date" ' . prowess_select_get_inline_style( $product_info_style ) . '>' . esc_html( $date ) . '</div>';
		}
	...
	public function productIdAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data = array();
				$data['value'] = $value['id'];
				$data['label'] = esc_html__( 'Id', 'prowess' ) . ': ' . $value['id'] . ( ( strlen( $value['title'] ) > 0 ) ? ' - ' . esc_html__( 'Title', 'prowess' ) . ': ' . $value['title'] : '' );
				$results[] = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( ! is_wp_error( $product ) ) {
				...
				if ( ! empty( $product_title ) ) {
					$product_title_display = ' - ' . esc_html__( 'Title', 'prowess' ) . ': ' . $product_title;
				}
				
				$product_id_display = esc_html__( 'Id', 'prowess' ) . ': ' . $product_id;
