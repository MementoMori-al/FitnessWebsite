<?php
namespace ProwessCore\CPT\Shortcodes\BlogSlider;
...
class BlogSlider implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		vc_map(
			array(
				'name'                      => esc_html__( 'Blog Slider', 'prowess' ),
				'base'                      => $this->base,
				...
				'params'                    => array(
					array(
						...
						'value'       => array(
							esc_html__( 'Slider', 'prowess' )            => 'slider',
							esc_html__( 'Carousel', 'prowess' )          => 'carousel',
							esc_html__( 'Carousel Centered', 'prowess' ) => 'carousel-centered'
						),
						'save_always' => true
					),
					array(
						...
						'heading'    => esc_html__( 'Number of Posts', 'prowess' )
					),
					array(
						'type'        => 'dropdown',
						'param_name'  => 'orderby',
						'heading'     => esc_html__( 'Order By', 'prowess' ),
						'value'       => array_flip( prowess_select_get_query_order_by_array() ),
						'save_always' => true
					),
					array(
						'type'        => 'dropdown',
					...
					array(
						...
						'value'      => array(
							...
							esc_html__( 'Thumbnail', 'prowess' ) => 'thumbnail',
					...
					array(
						...
						'group'       => esc_html__( 'Post Info', 'prowess' )
					),
					array(
						'type'        => 'dropdown',
					...
					array(
						...
						'heading'     => esc_html__( 'Enable Post Info Author', 'prowess' ),
					...
					array(
						...
						'heading'     => esc_html__( 'Enable Post Info Date', 'prowess' ),
						'value'       => array_flip( prowess_select_get_yes_no_select_array( false, true ) ),
						'save_always' => true,
						'group'       => esc_html__( 'Post Info', 'prowess' )
					),
					array(
						...
						'heading'     => esc_html__( 'Enable Post Info Category', 'prowess' ),
						'value'       => array_flip( prowess_select_get_yes_no_select_array( false, true ) ),
						'save_always' => true,
						'group'       => esc_html__( 'Post Info', 'prowess' )
					),
					array(
						...
						'heading'     => esc_html__( 'Enable Post Info Comments', 'prowess' ),
	...
	public function render( $atts, $content = null ) {
		...
		prowess_select_get_module_template_part( 'shortcodes/blog-slider/holder', 'blog', '', $params );
    ...
    public function blogListCategoryAutocompleteSuggester( $query ) {
        ...
        if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
            foreach ( $post_meta_infos as $value ) {
                ...
                $data['label'] = ( ( strlen( $value['category_title'] ) > 0 ) ? esc_html__( 'Category', 'prowess' ) . ': ' . $value['category_title'] : '' );
                $results[]     = $data;
            }
        }
        ...
        if ( ! empty( $query ) ) {
            ...
            if ( is_object( $category ) ) {
                ...
                if ( ! empty( $category_title ) ) {
                    $category_title_display = esc_html__( 'Category', 'prowess' ) . ': ' . $category_title;
                }
