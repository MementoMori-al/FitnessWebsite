 ...
 */
if ( ! class_exists( 'ProwessSidebar' ) ) {
	class ProwessSidebar {
		var $sidebars = array();
		var $stored = "";
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'prowess' );
		...
		function load_assets() {
			...
			wp_enqueue_script( 'qode-sidebar', QODE_ROOT . '/framework/admin/assets/js/qodef-ui/qodef-sidebar.js' );
			wp_enqueue_style( 'qode-sidebar', QODE_ROOT . '/framework/admin/assets/css/qodef-sidebar.css' );
		}
		...
		function template_add_widget_field() {
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_html__('Enter Name of the new Widget Area', 'prowess') . "' name='qodef-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_html__('Add Widget Area', 'prowess') . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function delete_sidebar_area() {
			...
			if ( ! empty( $_POST['name'] ) ) {
				...
				if ( ( $key = array_search( $name, $this->sidebars ) ) !== false ) {
					...
					update_option( $this->stored, $this->sidebars );
					echo esc_html__( "sidebar-deleted", 'prowess' );
				}
			}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'prowess_select_custom_widget_args', $args );
